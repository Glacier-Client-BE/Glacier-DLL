...
**Controls:** `G` or `M` opens the menu; `G` or `Escape` closes it (`M` only
opens — it never closes, so it can't double as "accidentally close what I
just opened"). Opening additionally requires the game to actually be in
gameplay (`cursorGrabbed()`), not chat/pause/inventory — GetAsyncKeyState
reports the physical key regardless of window focus, so before this, typing
"g" or "m" into a chat message could pop the menu open mid-sentence. `END`
unloads. Config at `%APPDATA%\Glacier\config.ini`.
...
| **8** | **Module catalog expansion.** Full scope is the whole Latite (~40 modules, `reference/latite`) and Flarial (~130 modules, `reference/flarial`) catalogs, minus a few excluded outright — `Doom` (an embedded game engine, not a Minecraft client feature), `SkinStealer` (copies another player's skin without consent), and server-specific utilities tied to one community (`HiveUtils`, `HiveStat`, `HiveModeCatcher`, `HiveTranslate`, `ZeqaUtils`). Being ported in batches, smallest-new-SDK-plumbing first. **Shipped so far:** Speedometer and Walk Distance (derived from `playerPosition()` deltas — there's no velocity accessor); Low Durability Warning (reuses `armor()`'s `durabilityFraction()`); Combo Counter (counts consecutive *attacks*, not confirmed hits — `recordAttack` has no server acknowledgement behind it — via an age-reset edge on `lastAttackDistance()`); Hotbar HUD and Inventory HUD (`GameSDK::hotbar()`/`inventory()`, mirroring `armor()`'s pattern over `PlayerInventory` instead of the ECS); Frame Time Display, Stopwatch, DVD Screen, Custom Crosshair (all zero new signatures — see their class comments for what got scoped down, e.g. Custom Crosshair has no image loading or enemy-highlight). **Next batch is unplanned** — whoever picks this up next should scan both `ModuleManager.cpp`/`Manager.cpp` for what's left and keep sorting by "needs a new SDK accessor" vs. "needs a new hook/signature" vs. "already covered". |
| **9** | **HUD widget visual pass — done for the current catalog.** A shared design-token system (`HudModule.h`) with per-widget padding/corner-radius/shadow settings, and a `TextHudModule` base so text widgets can't drift from each other. The HUD background defaults **off** everywhere, matching Latite's `HUDModule` (its `renderFrame`, a dark fill + border, is only ever used for the editor's "selected" highlight — never during normal play, confirmed by reading `reference/latite/src/client/feature/module/HUDModule.cpp`) — no module in the catalog opts back into one. Armor HUD and Hotbar HUD are chrome-less — bare icon, a thin colour-coded durability rail with no track, a shadowed stack-count badge — keyed off Latite's `ArmorHUD.cpp`. Keystrokes' per-key caps are intentional chrome, not decoration, and were checked against Latite's own `Keystrokes.cpp`, which draws the same filled rounded rect per key. **Re-open this phase whenever Phase 8 adds a widget** — the bar it has to clear is "does this box earn its place," and nothing added after this line has been checked against it yet. Nothing in this phase has been seen in-game; it's reasoned from the reference source, not observed — verify against a running client before trusting it. |

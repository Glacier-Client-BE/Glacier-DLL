...
| **8** | **Module catalog expansion.** The pack's ~30-widget list (Armor Bar, Bow Indicator, Death Coords, Direction HUD, Kill Counter, Offhand HUD, Status HUD, Target HUD, Chunk Map, EXP Calculator, Inventory HUD, Player List, Server Display, Timer HUD, and more) mostly needs SDK reads Glacier doesn't have yet (rotation, health, effects, network state) — deliberately deferred. **Shipped so far, using only accessors that already existed or were one small mirror of an existing one:** Speedometer and Walk Distance (derived from `playerPosition()` deltas — there's no velocity accessor); Low Durability Warning (reuses `armor()`'s `durabilityFraction()`, stays invisible until something crosses the threshold); Combo Counter (counts consecutive *attacks*, not confirmed hits — `recordAttack` has no server acknowledgement behind it — via an age-reset edge on `lastAttackDistance()`); Hotbar HUD (all 9 slots via the new `GameSDK::hotbar()`/`selectedHotbarSlot()`, mirroring `armor()` over `PlayerInventory` instead of the ECS). |

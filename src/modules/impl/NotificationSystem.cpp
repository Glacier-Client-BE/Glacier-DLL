...
void NotificationSystem::onRenderImGui(){
    ...
    for(auto& n: s_queue){
        ...
        float fade=1.f; if(age>dur-0.4f) fade=(dur-age)/0.4f;
        float slideX=0.f;
        if(m_settings.getBool("animate")&&age<0.2f) slideX=(1.f-(age/.2f))*(tw+20.f);
        ImGui::SetNextWindowPos({px+slideX,oy},ImGuiCond_Always);
        ImGui::SetNextWindowSize({tw,58.f}); ImGui::SetNextWindowBgAlpha(0.88f*fade);
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding,8); ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding,{0,0});
        char wid[32]; snprintf(wid,sizeof(wid),"##notif%p",&n);
        if(ImGui::Begin(wid,nullptr,f)){
            ImDrawList* dl=ImGui::GetWindowDrawList(); ImVec2 bp=ImGui::GetWindowPos();
            dl->AddRectFilled(bp,{bp.x+4,bp.y+58},ImAlphaBlendColors(n.accent,IM_COL32(0,0,0,0)&~0xFF000000|(int)(fade*255)<<24),2);
            ...
            dl->AddRectFilled({bp.x,bp.y+54},{bp.x+tw*prog,bp.y+58},n.accent,0,ImDrawFlags_RoundCornersBottom);
        }
